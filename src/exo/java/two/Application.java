...
public class Application implements Cloneable{
    ...
    public boolean addMark(float value, String subject, int rank){
        ...
        if (rank >= this.getMarks().size()){
            System.out.println("Result not found, creating a new student, with the following rank : "+this.marks.size());
    ...
    public boolean removeMark(int studentRank, String subject)throws IndexOutOfBoundsException{
        ...
        if (indexOfMark == -1){
            System.out.println("Result not found : no mark found for the specified subject concerning this student");
            return false;
        }
    ...
    public float average(int rank) throws IndexOutOfBoundsException{
        if (this.getMarks().size() < rank){
            throw new IndexOutOfBoundsException("Result not found");
        }
